...
import { MARKET_LISTINGS, createMyListing, loadMyListings, marketCategories, removeMyListing } from '../services/marketService'
import type { MarketListing, NavProps, WardrobeCategory, WardrobeItem } from '../types'
...
export default function MarketScreen({ onNavigate, initialCategory, initialColor, items, listingItem, onCompleteListing }: Props) {
  const [category, setCategory] = useState<WardrobeCategory | 'all'>(initialCategory ?? 'all')
  const [mode, setMode] = useState<'all' | 'buy' | 'trade'>('all')
  const [selected, setSelected] = useState<MarketListing | null>(null)
  const [draft, setDraft] = useState<WardrobeItem | null>(listingItem ?? null)
  const [choosingItem, setChoosingItem] = useState(false)
  const [listingType, setListingType] = useState<MarketListing['listingType'] | null>(listingItem ? null : null)
  const [price, setPrice] = useState('20')
  const [size, setSize] = useState(listingItem?.size ?? '')
  ...
  const saveListing = () => {
    if (!draft || !listingType) return
    const existing = mine.find(listing => listing.wardrobeItemId === draft.id)
    if (existing) { setNotice('Already listed on WE-AR Market.'); setDraft(null); setListingType(null); return }
    createMyListing(draft, listingType, { price: listingType === 'buy' ? Number(price) || undefined : undefined, size: size || draft.size || 'One size', condition, tradePreference: listingType === 'trade' ? tradePreference : undefined })
    setNotice(listingType === 'trade' ? 'Listed for trade. Your item is still in your wardrobe.' : 'Listed on WE-AR Market. Your item is still in your wardrobe.')
  ...
  return <div className="bg-[#f7f5f2] min-h-screen pb-10"><div className="h-12"/><div className="px-5">
    ...
    <div className="bg-[#1b4332] text-white rounded-[20px] p-4 mb-5"><p className="font-bold">Secondhand first</p><p className="text-xs text-white/75 mt-1">Buy secondhand, trade what you own, or give pieces a new home.</p></div>
    {notice && <div className="bg-[#fef9ec] border border-[#f0d98a] rounded-xl p-3 text-sm text-[#8a6200] mb-4">✓ {notice}</div>}
    {mine.length > 0 && <div className="mb-5"><p className="text-[10px] font-bold tracking-[.12em] text-[#999] mb-2">MY LISTINGS</p><div className="space-y-2">{mine.map(listing => <div key={listing.id} className="bg-white border border-[#edebe6] rounded-2xl p-3 flex gap-3"><img src={listing.image} alt="" className="size-14 object-contain bg-[#faf9f6] rounded-xl p-1"/><div className="min-w-0 flex-1"><p className="font-semibold text-sm truncate">{listing.title}</p><p className="text-[10px] font-bold text-[#1b4332] mt-0.5">{listing.listingType === 'trade' ? 'FOR TRADE' : `FOR SALE · $${listing.price}`}</p><p className="text-[10px] text-[#888] mt-0.5">{listing.listingType === 'trade' ? `Looking for: ${listing.tradePreference}` : 'Active listing'}</p></div><div className="flex flex-col items-end gap-1"><button onClick={() => setConfirmCompletion(listing)} className="text-[10px] font-bold text-[#1b4332]">{listing.listingType === 'trade' ? 'Mark as Traded' : 'Mark as Sold'}</button><button onClick={() => { removeMyListing(listing.id); setRevision(value => value + 1); setNotice('Listing removed. Your wardrobe item is unchanged.') }} className="text-[10px] text-[#a33]">Remove Listing</button></div></div>)}</div></div>}
    <div className="flex gap-2 mb-3"><button onClick={() => setMode('all')} className={`flex-1 rounded-xl py-2 text-sm font-bold ${mode === 'all' ? 'bg-[#111] text-white' : 'bg-white border border-[#edebe6]'}`}>All</button><button onClick={() => setMode('buy')} className={`flex-1 rounded-xl py-2 text-sm font-bold ${mode === 'buy' ? 'bg-[#111] text-white' : 'bg-white border border-[#edebe6]'}`}>Buy</button><button onClick={() => setMode('trade')} className={`flex-1 rounded-xl py-2 text-sm font-bold ${mode === 'trade' ? 'bg-[#111] text-white' : 'bg-white border border-[#edebe6]'}`}>Trade</button></div>
    <div className="flex gap-2 overflow-x-auto mb-5 pb-1">{marketCategories.map(item => <button key={item.value} onClick={() => setCategory(item.value)} className={`shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold ${category === item.value ? 'bg-[#1b4332] text-white' : 'bg-white border border-[#edebe6] text-[#666]'}`}>{item.label}</button>)}</div>
    <div className="grid grid-cols-2 gap-3">{listings.map(listing => <button key={listing.id} onClick={() => setSelected(listing)} className="bg-white border border-[#edebe6] rounded-[20px] overflow-hidden text-left"><div className="aspect-square bg-[#faf9f6] flex items-center justify-center p-3"><img src={listing.image} alt={listing.title} className="max-h-28 object-contain"/></div><div className="p-3"><div className="flex justify-between gap-1"><p className="font-semibold text-[12px] leading-tight">{listing.title}</p><span className="text-[10px] font-bold text-[#1b4332]">{listing.listingType === 'trade' ? 'Trade' : `$${listing.price}`}</span></div><p className="text-[10px] text-[#888] mt-1">{listing.size} · {listing.condition}</p><p className="text-[10px] text-[#aaa] mt-1">{listing.isMine ? 'Your listing' : listing.sellerName}</p></div></button>)}</div>
    {choosingItem && <div className="fixed inset-0 z-40 bg-black/40 flex items-end justify-center"><div className="w-full max-w-[430px] bg-[#f7f5f2] rounded-t-[28px] p-5"><p className="font-bold text-lg">List an Item</p><p className="text-xs text-[#888] mt-1">Choose an active wardrobe piece to list.</p><div className="grid grid-cols-3 gap-2 mt-4 max-h-64 overflow-y-auto">{items.filter(item => item.isActive !== false).map(item => <button key={item.id} onClick={() => { setDraft(item); setChoosingItem(false) }} className="bg-white border border-[#edebe6] rounded-xl p-2 text-center"><img src={item.image} alt="" className="h-12 w-full object-contain"/><p className="text-[10px] font-semibold truncate mt-1">{item.name}</p></button>)}</div><button onClick={() => setChoosingItem(false)} className="w-full py-3 text-sm text-[#888]">Cancel</button></div></div>}
